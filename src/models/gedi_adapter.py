...
class GediAdapter(GenerationMixin):
    ...
    def show_correction(self, sm, logits, corrected, old_logits):
        if self.tokenizer:
            ...
            for text, idx in zip(texts, toks):
                print(
                    "{:6d}: {:+2.2f} > {:+2.2f} {:20s}     [{:+2.2f} | {:+2.2f}]".format(
                        idx, lv[idx], cv[idx], text, pos_logits[idx], neg_logits[idx]
                    )
                )
            # print()
    ...
    def __call__(self, return_dict=True, **kwargs):
        ...
        stacked = torch.stack(
            [gedi_logits["gedi_pos"], gedi_logits["gedi_neg"]]
        )  # [2, batch, voc]
        ...
        if hasattr(self.gedi_model, "bias"):
            old_logits += self.gedi_model.bias.reshape(2, 1, 1).repeat(
                1, 1, old_logits.shape[-1]
            )
    ...
    def prepare_inputs_for_generation(self, input_ids, **kwargs):
        ...
        if kwargs.get("main_prefix") is not None and main_kwargs.get("past") is None:
            prefix = (
                kwargs["main_prefix"].unsqueeze(0).repeat(main_input_ids.shape[0], 1)
            )
            ...
            if main_kwargs.get("attention_mask") is not None:
                ...
                main_kwargs["attention_mask"] = torch.cat(
                    [mask_prefix, old_mask], dim=1
                )
                # print('main reshape mask:', old_mask.shape, main_kwargs['attention_mask'].shape)
        # print('main    ', self.tokenizer.decode(main_input_ids[0]))
        result["main"] = self.model.prepare_inputs_for_generation(
            main_input_ids, **main_kwargs
        )
        ...
        for k in ["gedi_pos", "gedi_neg"]:
            ...
            if kwargs.get("gedi_prepend"):
                ...
                prefix = (
                    torch.ones([input_ids.shape[0], 1], dtype=input_ids.dtype).to(
                        input_ids.device
                    )
                    * self.codes[k]
                )
            ...
            gedi_inputs = self.gedi_model.prepare_inputs_for_generation(
                new_input_ids, **gedi_args
            )
    ...
    def _update_model_kwargs_for_generation(
        ...
        result["past"] = {
            ...
            if "past" in result[k]
            and result[k]["past"] is not None
            and result[k]["past"][0] is not None
        }
        return result

    def paraphrase(self, text, n=None, max_length=128):
        inputs = tokenizer(text, return_tensors="pt", padding=True)["input_ids"].to(
            device
        )
        if max_length == "auto":
